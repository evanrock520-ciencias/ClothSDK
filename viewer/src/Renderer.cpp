...
void Renderer::cleanup() {
  ...
  if (m_debugEbo) glDeleteBuffers(1, &m_debugEbo);
}
...
void Renderer::buildCapsuleLines(const Eigen::Vector3d& start,
  ...
  if (std::abs(axisN.dot(up)) > 0.99) up = Eigen::Vector3d(1, 0, 0);
...
void Renderer::buildPlaneLines(const Eigen::Vector3d& origin,
  ...
  if (std::abs(n.dot(u)) > 0.99) u = Eigen::Vector3d(0, 1, 0);
  ...
  for (int i = 0; i <= divs; ++i) {
    ...
    unsigned int idx = base + static_cast<unsigned int>(verts.size() / 3) - base;
...
void Renderer::renderColliders(const Tissu::World& world, const Camera& camera) {
  ...
  for (const auto& cPtr : colliders) {
    ...
    if (auto* s = dynamic_cast<const SphereCollider*>(cPtr.get())) {
      buildSphereLines(s->getCenter(), s->getRadius(), allVerts, allIndices);
      drawCalls.push_back(
          {idxOffset, static_cast<unsigned int>(allIndices.size() - idxOffset),
           Eigen::Vector3f(0.30f, 0.60f, 1.00f)});  // blue

    } else if (auto* cap = dynamic_cast<const CapsuleCollider*>(cPtr.get())) {
      ...
      buildPlaneLines(pl->getOrigin(), pl->getNormal(), allVerts, allIndices);
      drawCalls.push_back(
          {idxOffset, static_cast<unsigned int>(allIndices.size() - idxOffset),
      ...
      if (wverts.empty() || tris.empty()) continue;
      ...
      for (const auto& tri : tris) {
        ...
        allIndices.push_back(a); allIndices.push_back(b);
        allIndices.push_back(b); allIndices.push_back(c);
        allIndices.push_back(c); allIndices.push_back(a);
      }
      drawCalls.push_back(
          {idxOffset, static_cast<unsigned int>(allIndices.size() - idxOffset),
  ...
  for (const auto& dc : drawCalls) {
    ...
    glDrawElements(
        GL_LINES, static_cast<GLsizei>(dc.indexCount), GL_UNSIGNED_INT,
        reinterpret_cast<const void*>(dc.indexOffset * sizeof(unsigned int)));
  }
