...
void Application::processInput() {
    ...
    if (sIsPressed && !sWasPressed) {
        ...
        std::string name = "data/snapshots/" + cloth_name + "On" +
    ...
    if (gIsPressed && !gWasPressed) {
        int frame = m_solver->getCurrentFrame();
        std::string name =
            "data/states/frame" + std::to_string(frame) + ".tissu";
...
void Application::render() {
    ...
    if (m_hoveredParticleIndex != -1) {
        ...
        if (screenPos.x >= 0) {
            ...
            drawList->AddCircle(screenPos, 8.0f, IM_COL32(255, 255, 255, 200),
                                0, 2.0f);
    ...
    if (m_hoveredColliderVertexIndex != -1 && m_hoveredColliderIndex != -1) {
        ...
        if (m_hoveredColliderIndex < static_cast<int>(colliders.size())) {
            ...
            if (mc) {
                ...
                if (m_hoveredColliderVertexIndex <
                    static_cast<int>(wv.size())) {
...
void Application::drawUI() {
    ...
    if (ImGui::CollapsingHeader("Configuration IO",
        ...
        if (ImGui::Button("Load physics")) {
            try {
                ConfigLoader::loadPhysics(m_configPhysicsPath, *m_solver,
                                          *m_world);
        ...
        if (ImGui::Button("Save Physics Settings")) {
            try {
                ConfigLoader::savePhysics("exported_physics.json", *m_solver,
                                          *m_world, "exported");
        ...
        if (ImGui::Button("Save Scene")) {
            try {
                SceneExporter::saveScene("exported_scene.json", "exported",
                                         *m_solver, *m_world);
    ...
    if (ImGui::CollapsingHeader("Shader & Materials",
                                ImGuiTreeNodeFlags_DefaultOpen)) {
        auto& meshes = m_renderer->getClothMeshes();
        for (size_t i = 0; i < meshes.size(); ++i) {
            float col[4] = {meshes[i].color[0], meshes[i].color[1],
                            meshes[i].color[2], meshes[i].color[3]};
        ...
        if (m_hoveredParticleIndex != -1) {
            ...
            if (m_hoveredColliderIndex < static_cast<int>(colliders.size())) {
                ...
                if (mc && m_hoveredColliderVertexIndex <
                    ...
                    ImGui::Text("Position: (%.4f, %.4f, %.4f)", vtx.x(),
                                vtx.y(), vtx.z());
                }
            }
        } else {
...
void Application::syncVisualTopology() {
    if (!m_world || !m_renderer) {
        Logger::warn(
            "Cannot sync topology: World or Renderer not initialized.");
        return;
    }
...
int Application::findClosestColliderVertex(const Ray& ray, const World& world) {
