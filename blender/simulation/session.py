...
from _cloth_sdk_core import GravityForce, ClothMaterial, ClothMesh, Cloth, AerodynamicForce
from .bridge import get_simulation, Fabric, Material
...
def _write_local_vertex_positions(obj, local_positions):
    ...
    if len(mesh.vertices) != len(local_positions):
        print(
            f"[Tissu] Warning: cannot restore {obj.name}, vertex count mismatch "
...
def restore_fabric_rest_positions(context):
    ...
    for obj_name, local_positions in _fabric_rest_positions.items():
        ...
        if obj and obj.type == 'MESH':
            if _write_local_vertex_positions(obj, local_positions):
                restored_count += 1

    if restored_count:
        print(f"[Tissu] Restored rest positions for {restored_count} fabric object(s).")
...
def get_triangulated_mesh_data(obj, local=False):
    """Evaluates the object modifiers, triangulates the mesh, and returns vertices in local/world space, triangle indices, and pinned vertex indices."""
...
def update_blender_mesh(obj, world_positions):
    """Converts world space coordinates back to the object's local space and updates the viewport mesh."""
    ...
    if len(mesh.vertices) == vertex_count:
        ...
        print(f"[Tissu] Warning: mesh {obj.name} vertex count mismatch ({len(mesh.vertices)} vs {vertex_count}). Make sure the mesh is triangulated.")

def sync_simulation(context):
    """Scans the Blender scene, clears existing simulation, and loads meshes as cloths into the solver."""
    sim = get_simulation()

    # Keep a stable rest pose so "Reset Simulation" can return meshes to their original state.
    ...
    for obj in collider_objs:
        ...
        if obj.tissu_collider_type == 'PLANE':
            ...
            local_normal = np.array([0.0, 1.0, 0.0], dtype=np.float64) # pointing along local Z in Blender -> Y in Tissu
            ...
            # Capsule segment length along local Z axis in Blender (local Y in Tissu)
    ...
    for seam in seams:
        ...
        if fabric_A and fabric_B:
            ...
            if v_idx_A < len(mapA) and v_idx_B < len(mapB):
                ...
                print(f"[Tissu] Stitched {obj_name_A}:{v_idx_A} to {obj_name_B}:{v_idx_B}")

    print(f"[Tissu] Simulation synced with {len(fabric_objs)} fabric(s) and {len(collider_objs)} collider(s).")

def step_simulation(context, dt=1.0/60.0):
    """Steps the physical simulation solver and updates viewport meshes with new vertex coordinates."""
    sim = get_simulation()
...
def frame_change_pre_handler(scene):
    """Handles simulation updates synchronized with the Blender timeline frame changes."""
