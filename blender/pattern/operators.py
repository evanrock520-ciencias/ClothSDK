            ...
            if obj and obj.type == 'MESH' and obj.mode == 'EDIT':
                ...
                if vg:
                    ...
                    if deform_layer:
                        ...
                        for v in bm.verts:
                            if v.select:
                                if group_idx in v[deform_layer]:
                                    del v[deform_layer][group_idx]
                                    count += 1
                        bmesh.update_edit_mesh(obj.data)
                        self.report({'INFO'}, f"Unpinned {count} vertices in {obj.name}.")
        return {'FINISHED'}
...
class TISSU_OT_AddSeam(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        new_seam = [selected_verts[0][0], selected_verts[0][1], selected_verts[1][0], selected_verts[1][1]]
        
        for seam in seams:
            if (seam[0] == new_seam[0] and seam[1] == new_seam[1] and seam[2] == new_seam[2] and seam[3] == new_seam[3]) or \
        ...
        self.report({'INFO'}, f"Added seam between {new_seam[0]}:{new_seam[1]} and {new_seam[2]}:{new_seam[3]}")
        return {'FINISHED'}
...
class TISSU_OT_PatternFromSelectedMesh(bpy.types.Operator):
    ...
    def execute(self, context):
        self.report({'INFO'}, "Pattern from selected mesh is not implemented in the physics MVP.")
        return {'FINISHED'}
