...
TEST(ConstraintGraph, EveryBatchIsAnIndependentSet) {
    ...
    for (int idx = 0; idx < size - 3; idx++) {
        constraints.push_back(std::make_unique<DistanceConstraint>(idx, idx + 1, 1.0, 0.0));
        constraints.push_back(std::make_unique<DistanceConstraint>(idx, idx + 2, 1.0, 0.0));
        constraints.push_back(std::make_unique<DistanceConstraint>(idx, idx + 3, 1.0, 0.0));
    }
