...
bool StateSerializer::load(const std::string &path, Solver& solver, World& world) {
    ...
    if (particles.size() != count) {
        Logger::error("Particles size mistmatch: '" + std::to_string(count) + "' rather than '" + std::to_string(particles.size()) + "'");
        return false;
    }
    ...
    if (constraints.size() != constraintsCount) {
        Logger::error("Constraints size mistmatch: '" + std::to_string(constraintsCount) + "' rather than '" + std::to_string(constraints.size()) + "'");
        return false;
    }
