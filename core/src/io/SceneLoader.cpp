...
void SceneLoader::loadScene(const std::string& filepath, Solver& solver, World& world)    
{
...
void SceneLoader::loadCollider(const nlohmann::json& collider, World& world) {
    ...
    if (type == "plane") {
        Eigen::Vector3d origin = ConfigLoader::jsonToVector(collider.value("origin", nlohmann::json{0.0, 0.0, 0.0}));
        Eigen::Vector3d normal = ConfigLoader::jsonToVector(collider.value("normal", nlohmann::json{0.0, 1.0, 0.0}));

        world.addPlaneCollider(origin, normal, friction);
    }

    else if (type == "sphere") {
        Eigen::Vector3d center = ConfigLoader::jsonToVector(collider.value("center", nlohmann::json{0.0, 0.0, 0.0}));
        ...
        Eigen::Vector3d start = ConfigLoader::jsonToVector(collider.value("start", nlohmann::json{0.0, 0.0, 0.0}));
        Eigen::Vector3d end = ConfigLoader::jsonToVector(collider.value("end", nlohmann::json{0.0, 4.0, 0.0}));
        double radius = collider.value("radius", 1.0);
        
        world.addCapsuleCollider(start, end, radius, friction);
    }

    else throw std::invalid_argument("Unknown collider " + type);
