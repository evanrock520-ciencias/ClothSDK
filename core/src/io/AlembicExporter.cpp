...
bool AlembicExporter::open(
    ...
    try {
        m_impl->archive = std::make_unique<OArchive>(
        ...
        for (size_t i = 0; i < names.size(); ++i) {
            ...
            ExportedMesh em;
            em.xform = std::make_unique<OXform>(*m_impl->archive,
                                                name + "_xform", tsIndex);
            em.mesh =
                std::make_unique<OPolyMesh>(*em.xform, name + "_mesh", tsIndex);
            ...
            for (size_t local_idx = 0; local_idx < part_indices.size();
                 ++local_idx) {
                global_to_local[part_indices[local_idx]] =
            ...
            initialSample.setFaceIndices(Abc::Int32ArraySample(
                local_indices.data(), local_indices.size()));
