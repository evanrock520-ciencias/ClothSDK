...
void ConstraintGraph::buildFrom(const std::vector<std::unique_ptr<Constraint>>& constraints, unsigned int seed = 42) {
        ...
        #pragma omp parallel
        {
            ...
            for (int idx = 0; idx < m_nodeCount; idx++) {
                ...
                for (int neighbor : neighbors(idx)) {
                    if (!done[neighbor] && m_weights[neighbor] > m_weights[idx]) {
                        wins = false;
                        break;
                    }
                }
