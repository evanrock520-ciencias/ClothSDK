...
void Particle::addForce(const Eigen::Vector3d& force) {
    const double fx = force.x() * inverseMass;
    const double fy = force.y() * inverseMass;
    const double fz = force.z() * inverseMass;
...
void Particle::integrate(double deltaTime) {
    ...
    m_position =
        m_position + velocity + m_acceleration * (deltaTime * deltaTime);
