...
BendingConstraint::BendingConstraint(int idA, int idB, int idC, int idD,
                                     double restAngle, double compliance)
    : m_idA(idA), m_idB(idB), m_idC(idC), m_idD(idD), m_restAngle(restAngle) {
...
void BendingConstraint::solve(std::vector<Particle>& particles, double dt) {
    ...
    Eigen::Vector3d gradA = ((xC - xB).dot(e) * invLen2) * gradC +
                            ((xD - xB).dot(e) * invLen2) * gradD;

    Eigen::Vector3d gradB = ((xA - xC).dot(e) * invLen2) * gradC +
    ...
    double denom = wA * gradA.squaredNorm() + wB * gradB.squaredNorm() +
                   wC * gradC.squaredNorm() + wD * gradD.squaredNorm() + alpha;
