...
void BVH::queryRecursive(int nodeIdx, const Eigen::Vector3d& point,
    ...
    if (node.bbox.squaredExteriorDistance(point) > squaredRadius)
        return;
...
int BVH::closestRecursive(int nodeIdx, const Eigen::Vector3d& point,
    ...
    if (node.isLeaf()) {
        ...
        for (int i = 0; i < node.primitiveCount; ++i) {
            ...
            Eigen::Vector3d closest = closestPointOnTriangle(
                point, vertices[tri.a], vertices[tri.b], vertices[tri.c]);
