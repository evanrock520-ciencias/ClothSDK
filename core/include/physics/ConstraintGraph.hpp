...
public:
    ...
    void buildFrom(const std::vector<std::unique_ptr<Constraint>>& constraints, unsigned int seed);

    inline int nodeCount() const { return m_nodeCount; }
    inline int edgeCount() const { return m_edgeCount; }

    const std::vector<int>& neighbors(int node) const { return m_adjacency[node]; }
