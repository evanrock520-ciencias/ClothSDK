...
public:
   ...
   * @param compliance Physical compliance (inverse of the stiffness), measured
   * in m/N.
   */
  DistanceConstraint(int idA, int idB, double restLength, double compliance);

  /**
   * @brief Solves the constraint by updating particle positions and the
   * Lagrange multiplier. Following the XPBD formulation, the Lagrange
   * multiplier increment @f$ \Delta \lambda @f$ is:
   * @f[
   * \Delta \lambda = \frac{-C(\mathbf{p}) - \tilde{\alpha} \lambda}{\sum w_i
   * |\nabla C_i|^2 + \tilde{\alpha}}
   * @f]
   * where @f$ \tilde{\alpha} = \frac{\alpha}{\Delta t^2} @f$ is the
   * time-step-corrected compliance.
   *
